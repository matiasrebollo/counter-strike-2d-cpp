...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        new_skt = socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
        if (new_skt == -1) {
            continue;
        }
         ...
         * */
        s = connect(new_skt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(new_skt, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(new_skt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
