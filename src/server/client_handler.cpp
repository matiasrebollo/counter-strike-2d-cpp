...
void ClientHandler::manage_create_game(const CreateGameDTO&) {
    std::shared_ptr<CS2DGame> game = this->server_monitor.create_new_game();
    if (!this->in_game() && this->username != "") {
        ...
        ClientSender sender(this->protocol, this->username);
...
void ClientHandler::manage_join_game(const JoinGameDTO& dto) {
    std::shared_ptr<CS2DGame> game = this->server_monitor.join_game(dto.gamename);
    if (!this->in_game() && this->username != "") {
        this->is_in_game = true;
